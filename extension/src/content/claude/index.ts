/**
 * Claude platform module — orchestrates detection, observation, and injection.
 *
 * Responsibilities:
 ...
 *  - Include projectContext in status messages (Claude-specific)
 */

import { PLATFORMS } from '@agenttailor/shared';
import type { ExtensionMessage } from '../../shared/types.js';
import { getConversationId, isInputReady, isProjectContext } from './detector.js';
  ...
  if (msg.type === 'INJECT_CONTEXT') {
    ...
    const result: ExtensionMessage = success
      ...
      : {
          ...
          success: false,
          error: 'Input element not found, Claude is responding, or injection failed',
        };
    sendResponse(result);
...
/** Initialize the Claude content module. Safe to call once per page load. */
export function initClaude(): void {
  console.log('AgentTailor: Initializing Claude platform module');
  ...
  console.log('AgentTailor: Claude platform module ready', state);
}
